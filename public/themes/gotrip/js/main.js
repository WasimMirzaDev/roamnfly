...
$(document).on('change', '.countTravellers', function() {
    var adults = $("#seat_type_adults").val()+' '+'Adults';
    var children = $("#seat_type_children").val()+' '+'Children';
    var infants = $("#seat_type_infants").val()+' '+'Infants';

    var selectedValue = $(this).attr('data-name');

    var renderHtml = selectedValue+' '+adults+' '+children+' '+infants;
    $("#renderTravellerCount").html(renderHtml);
});

$(document).on('click','.getMultiRow',function(){
    var type = $("#travel_type").val();
    if(type == 'Multicity'){
        $("#multiCityDiv1").removeClass('d-none');
        $("#multiCityDiv1").find('input, select').prop('disabled', false);
        $("#flightSearch").addClass('mt-20');
    }else{
        $("#multiCityDiv1").addClass('d-none');
        $("#multiCityDiv1").find('input, select').prop('disabled', true);
        $("#flightSearch").removeClass('mt-20');
    }
    
  });
  $(document).ready(function() {
    let cityCount = 2; // Initialize city count

    // Event handler for adding a city
    $(document).on('click', '.addCity', function() {
        if (cityCount < 6) {
            cityCount++; // Increment the count
            let newRow = $(`#multiCityDiv${cityCount}`); // Select the next city row
            newRow.removeClass('d-none'); // Show the hidden row
            newRow.find('input, select').prop('disabled', false); // Enable inputs in the row

            // Change button to remove city
            newRow.find('.addCity').removeClass('addCity').addClass('removeCity').html('<i class="icon-minus text-15 mr-10"></i> Remove City');
        }
    });

    // Event handler for removing a city
    $(document).on('click', '.removeCity', function() {
        let cityDiv = $(this).closest('.col-md-12'); // Find the parent city div
        cityDiv.addClass('d-none'); // Hide the row
        cityDiv.find('input, select').val(''); // Clear input values
        cityDiv.find('input, select').prop('disabled', true); // Disable inputs in the row

        // Reset button to add city
        $(this).removeClass('removeCity').addClass('addCity').html('<i class="icon-plus text-15 mr-10"></i> Add Another City');
        cityCount--; // Decrement the city count
    });

    // Initially disable inputs in hidden city rows
    $('.col-md-12.d-none').find('input, select').prop('disabled', true);
});
