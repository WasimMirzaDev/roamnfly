                ...
                <div class="galleryGrid -type-1 relative">
                    @foreach($galleries as $key=>$item)
                        ...
                        <div class="galleryGrid__item">
                            {{-- @if ($item['sz'] == 'XL') --}}
                                <img src="{{$item['url']}}" alt="{{ __("Gallery") }}" class="rounded-4">
                            {{-- @endif --}}
                        </div>
                    @endforeach
                    ...
                    <div class="absolute h-full col-12 z-2 px-20 py-20 d-flex justify-end items-end gotrip-banner">
                        ...
                        @foreach($galleries as $key=>$item)
                            @if($key == 0)
                                <a href="{{$item['url']}}" class="button -blue-1 px-24 py-15 bg-white text-dark-1 js-gallery" data-gallery="gallery2">
                                    {{ __('See All :count Photos',['count'=>count($galleries)]) }}
                                </a>
                            @else
                                <a href="{{$item['url']}}" class="js-gallery" data-gallery="gallery2"></a>
                            @endif
                        @endforeach
                    </div>
                </div>
