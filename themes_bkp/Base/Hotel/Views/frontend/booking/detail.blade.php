	...
	<div class="booking-review-content">
		<div class="review-section">
			<div class="service-info">
				<div>
					<h3 class="service-name"><a href="{{ \Modules\Hotel\Services\HotelService::getDetailUrl($service['id']) }}">{{ $service['name'] }}</a></h3>
					@if($service['ad']['adr'])
				...
				<div>
					@if($image_url = $service['img'])
						{{-- @if(!empty($disable_lazyload)) --}}
							<img src="{{$service['img'][0]['url']}}" class="img-responsive" alt="{{ $service['name'] }}">
						{{-- @else
			...
			<ul class="review-list">
				...
				@if(!empty($rooms))
					@foreach($rooms as $room)
						<li class="flex-wrap">
							<div class="label">{{$room->title}} * {{$room->number}}</div>
							<div class="val">
				...
				@if(!empty($rooms))
					...
					@foreach($rooms as $room)
						<li class="mb-3 pb-1 border-bottom">
							<h6 class="label text-center font-weight-bold mb-1">{{$room->title}} * {{$room->number}}</h6>
							<div>
								@includeIf("Hotel::frontend.booking.detail-room",['room'=>$room,'booking'=>$booking])
							</div>
