...
{    
    ...
    private function setSearchParams($searchParams)
    {
        return  [
                    'searchQuery' => [
                        'checkinDate' => Carbon::parse($searchParams['start'])->format('Y-m-d'),
                        'checkoutDate' => Carbon::parse($searchParams['end'])->format('Y-m-d'),
                        'roomInfo' => [
                            [
    ...
    {
        try {
            $response = Http::withHeaders(['apikey' => $this->apiToken])->post($this->getEndpoint('hotel/book','oms'), $this->setBookParams($request, $roomDetail, $bookingDetail));
    ...
    private function setBookParams($request, $roomDetail, $bookingDetail)
    {
        $user   = $request->all();
        return  [
                    'bookingId'             => $this->getRoomDetailFromCache($roomDetail->api_room_id)['bookingId'],
                    'roomTravellerInfo'     => [
                                                    [
                                                        'travellerInfo' => [
                                                                                [
                    ...
                    'type'                  => $bookingDetail->object_model,
                    'paymentInfos'          => [
                                                    [
                                                        'amount' => [ $roomDetail->price * $roomDetail->number ]
                                                    ]
                                                ]
                ];
    }
