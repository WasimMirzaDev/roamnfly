        ...
        @if(!empty($list_item))
            <div class="row y-gap-20">
                ...
                @foreach($list_item as $key=>$item)
                    <div data-anim="slide-up delay-{{$stt}}" class="{{ ($style ?? '' == 'style1') ? 'col-lg-4 col-sm-6' : 'col-md-3' }}">

                        <div class="ctaCard -type-1 asdsad rounded-4 @if(empty($item['offer_overLay'])) -no-overlay @endif">
                            ...
                            <div class="ctaCard__content {{ ($style ?? '' == 'style1') ? 'py-50 px-50' : 'py-70 px-30' }}  lg:py-30 lg:px-30">

                                @if(!empty($item['featured_text']))
                                    <div class="text-15 fw-500 text-white mb-10">{{ $item['featured_text'] }}</div>
                                @endif
                                <h4 class="{{ $style ?? '' == 'style1' ? 'text-30 xl:text-24' : 'text-15 lg:text-26' }} text-white">{!! clean($item['title']) !!}</h4>
