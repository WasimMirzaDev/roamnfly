    ...
    <div id="booking-detail-{{$booking->id}}" class="tab-pane active">
        ...
        <div class="more-booking-review">
            @if ($booking->object_model == 'hotel')
                @include (\Modules\Hotel\Services\HotelService::$checkout_booking_detail_file)
            @elseif ($booking->object_model == 'flight')
                @include (\Modules\Flight\Services\FlightService::$checkout_booking_detail_file)
            @else
