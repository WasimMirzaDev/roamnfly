            ...
            <div class="row booking-success-detail">
                ...
                <div class="col-md-4">
                    @if ($booking->travel_type == 'Round Trip')
                        @include ('Flight::frontend/booking/returnDetail',['disable_lazyload'=>1])
                    @elseif($booking->travel_type == 'Multicity')
                        @include ('Flight::frontend/booking/multiFlightDetail',['disable_lazyload'=>1])
                    @else
                        @include ($service->checkout_booking_detail_file ?? ucwords($booking->object_model).'::frontend/booking/detail',['disable_lazyload'=>1])
                    @endif
                </div>
            </div>
        </div>
