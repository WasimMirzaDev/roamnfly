                                ...
                                @if(count($recent_bookings) > 0)
                                    @foreach($recent_bookings as $booking)
                                        <tr>
                                            <td>#{{$booking->id}}</td>
                                            <td>
                                                @if(get_bookable_service_by_id($booking->object_model) and $service = ($booking->object_model != 'hotel' && $booking->object_model!='flight') ? $booking->service : $booking->object_model)
                                                    @if ($service == 'hotel')
                                                        <a target="_blank" href="{{ \Modules\Hotel\Services\HotelService::getDetailUrl($booking->api_id) }}">
                                                            {{ $booking->title }}
                                                        </a>
                                                    @elseif ($service == 'flight')
                                                        <a target="_blank" href="{{ \Modules\Flight\Services\FlightService::getDetailUrl($booking->api_id) }}">
                                                            {{ $booking->title }}
                                                        </a>
                                                    @else
